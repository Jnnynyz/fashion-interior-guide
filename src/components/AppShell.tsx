...
import { Sparkles, Upload, Clock, LogOut, LogIn, CreditCard } from "lucide-react";
          ...
          <div className="flex items-center gap-3">
            {user && !entLoading && (
              <Link
                ...
                title={hasSubscription ? "Unlimited plan" : `${credits} credits remaining`}
              >
                <Sparkles className="h-3 w-3 text-accent" />
                {hasSubscription ? "Unlimited" : `${credits} left`}
              </Link>
            )}
            {user ? (
              <button
